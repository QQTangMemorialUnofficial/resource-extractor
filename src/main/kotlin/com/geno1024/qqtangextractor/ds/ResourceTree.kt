...
object ResourceTree
{
    ...
    fun listFilesWithinZip(file: File, tabDepth: Int = 0): String = "${"    ".repeat(tabDepth)}- [ ] ${file.toRelativeString(File(Settings.base)).replace("\\", "/")}\n${if (file.isDirectory) file.listFiles()?.joinToString(separator = "") { listFiles(it, tabDepth + 1) } else ""}"

    fun toTreeDiagram() = listFiles(File(Settings.base))

    fun toTreeDiagramWithinZip() = listFilesWithinZip(File(Settings.base))
}
